    ...
    link: function ($scope, $elm, $attrs, uiGridCtrl) {
      ...
      $scope.hideMenu = function( broadcastTrigger ) {
        if ($scope.col && $scope.col.grid) {
          gridUtil.focus.byId($scope.col.uid + '-menu-button', $scope.col.grid);
          // This ensures that the next time hideMenu is called (for example from a column sort) the focus isn't bounced arround weirdly
          delete $scope.col;
        }
        $scope.menuShown = false;
        if ( !broadcastTrigger ){
      ...
      $scope.hideColumn = function () {
        ...
        //Since we are hiding this column the default hide will fail so we need to focus somewhere else.
        gridUtil.focus.byId('grid-menu', $scope.grid);
      };
    },
