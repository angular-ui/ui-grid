  ...
  angular.module('ui.grid')
    ...
    .factory('GridRenderContainer', ['gridUtil', 'uiGridConstants', function(gridUtil, uiGridConstants) {
      ...
      GridRenderContainer.prototype.adjustRows = function adjustRows(scrollTop, scrollPercentage, postDataLoaded) {
        ...
        var vertLen = self.getVerticalScrollLength();

        //// Calculate the scroll percentage according to the scrollTop location, if no percentage was provided
        if ((typeof(scrollPercentage) === 'undefined' || scrollPercentage === null) && scrollTop) {
          scrollPercentage = scrollTop / vertLen ;
        }

        var theoreticalRowIndex = 0;
        var heightAtFirstRow = 0;
        var currentHeight = 0;
        var i = 0;
        ...
        rowCache.forEach(function(r){
          if ( scrollTop > currentHeight && scrollTop < currentHeight + r.height){
            ...
            heightAtFirstRow = currentHeight;
          }
        ...
        var rowIndex = 0; // OLD = Math.ceil(Math.min(maxRowIndex, maxRowIndex * scrollPercentage));
        rowIndex = Math.min(maxRowIndex,theoreticalRowIndex);

        //TODO: do we need to fix here?
