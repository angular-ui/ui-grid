        ...
        compile: function() {
            return {
                pre: function($scope, iElement) {
                    var html;
                    var columnExpr = 'row.entity[' + angular.toJson($scope.col.field) + ']';
                    var cellTemplate = $scope.col.cellTemplate.replace(COL_FIELD, columnExpr);

                    if ($scope.col.enableCellEdit) {
                        html =  $scope.col.cellEditTemplate;
                        html = html.replace(CELL_EDITABLE_CONDITION, $scope.col.cellEditableCondition);
                        html = html.replace(DISPLAY_CELL_TEMPLATE, cellTemplate);
                        html = html.replace(EDITABLE_CELL_TEMPLATE, $scope.col.editableCellTemplate.replace(COL_FIELD, columnExpr));
                    } else {
                        html = cellTemplate;
                    }
