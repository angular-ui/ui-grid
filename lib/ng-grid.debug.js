...
* Compiled At: 01/07/2013 11:51:08
***********************************************/
...
ng.SelectionService = function(grid) {
	...
	self.deselectAllRows = function(){	
		var rowCache = grid.rowFactory.rowCache;
		for(var i = 0, rowCount = 0; i < rowCache.length; i++){
			if(rowCache[i].entity[SELECTED_PROP] && rowCache[i].selected){
				rowCache[i].entity[SELECTED_PROP] = false;
				rowCache[i].selected = false;
				if(++rowCount == self.selectedItems.length) break;
			}
		}
